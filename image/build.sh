...
OUTPUT_IMAGE="reticulumhf_v1.0.img"
...
# Use public DNS (not host's - may be Tailscale/VPN which won't work in chroot)
echo "nameserver 1.1.1.1" | sudo tee "$MOUNT_DIR/etc/resolv.conf" > /dev/null
echo "nameserver 8.8.8.8" | sudo tee -a "$MOUNT_DIR/etc/resolv.conf" > /dev/null
...
# Install wait-for-port utility (used by service files, replaces netcat dependency)
sudo cp "$PROJECT_DIR/scripts/wait-for-port" "$MOUNT_DIR/usr/local/bin/"
sudo chmod +x "$MOUNT_DIR/usr/local/bin/wait-for-port"
...
# Force IPv4 to avoid IPv6 connectivity issues with RPi repos
